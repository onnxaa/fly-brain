"""Lesions: (1) silent KC -> anosmia, (2) silent DAN -> no learning, (3) half MBON -> degradation.
Predictions must hold: d~0 after KC lesion; d unchanged after 1 DAN-less trial; half MBON weaker separation."""
import numpy as np
...
# 1) KC lesion: zero K->M weights (MB output cut)
w = api.wM.copy(); api.wM[api.km_mask] = 0.0
lA = api.step(odor="A")["MB_pref"]; lB = api.step(odor="B")["MB_pref"]
print(f"KC-lesion: A={lA:.1f} B={lB:.1f} d={lA-lB:.1f} (expect d~0, anosmia)", flush=True)
api.wM[:] = w
# 2) DAN lesion: training with no reward/punish changes nothing
ei0 = api.step(odor="A")["EI_sum"]
api.train(odor="A", reward=0.0, punish=0.0)
ei1 = api.step(odor="A")["EI_sum"]
print(f"DAN-lesion (reward=0): EI {ei0:.0f}->{ei1:.0f} (expect identical)", flush=True)
# 3) half MBON cut
...
print(f"1/2 MBON: A={hA:.1f} B={hB:.1f} d={hA-hB:.1f} (expect |d| smaller than baseline {abs(base_A-base_B):.1f})", flush=True)
