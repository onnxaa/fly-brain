"""ALPN patterns from ORN->ALPN weights for DoOR odors (for injection into Brian-MB)."""
...
print(f"edges ->ALPN from outside ALPN: {m.sum()}", flush=True)
out = {}
base_corr = {}
for name in ["geosmin", "hexanone3", "co2", "butanedione", "methyl_salicylate", "ethyl_hexanoate"]:
    ...
    out[name] = v
    print(f"{name}: ALPN active={(v>0).sum()}/{len(v)} max={v.max():.1f}", flush=True)
import itertools
