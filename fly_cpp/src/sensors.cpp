...
std::vector<float> FlyBrain::sample_at(const std::vector<float>& g, int H, int Wd,
                                      const std::vector<float>& cxs,
                                      const std::vector<float>& cys) const {
    // same 3x3 Gaussian RF + bilinear as sample_R, arbitrary coords
    std::vector<float> b(H * Wd);
    auto at = [&](int y, int x) -> float {
        y = y < 0 ? 0 : (y >= H ? H - 1 : y);
        x = x < 0 ? 0 : (x >= Wd ? Wd - 1 : x);
    ...
    for (int y = 0; y < H; y++)
        for (int x = 0; x < Wd; x++)
            b[(size_t)y * Wd + x] =
                (4 * at(y, x) + 2 * (at(y - 1, x) + at(y + 1, x) + at(y, x - 1) + at(y, x + 1)) +
                 at(y - 1, x - 1) + at(y - 1, x + 1) + at(y + 1, x - 1) + at(y + 1, x + 1)) / 16.0f;
    std::vector<float> o(cxs.size());
    for (size_t i = 0; i < cxs.size(); i++) {
        double xs = (double)cxs[i] * (Wd - 1), ys = (double)cys[i] * (H - 1);
        int64_t x0 = (int64_t)std::floor(xs), y0 = (int64_t)std::floor(ys);
        x0 = std::max<int64_t>(0, std::min<int64_t>(x0, Wd - 1));
        y0 = std::max<int64_t>(0, std::min<int64_t>(y0, H - 1));
        int64_t x1 = std::min<int64_t>(x0 + 1, Wd - 1), y1 = std::min<int64_t>(y0 + 1, H - 1);
