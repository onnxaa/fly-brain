"""APL: kc = relu(drive - g*mean(drive)) przed top-k. Sweep g na obwodzie MB (szybki)."""
...
for g in [0.0, 0.5, 0.9, 1.0, 1.2, 1.5]:
    ...
    print(f"g={g}: overlap={ov}/{k} corr={corr:.2f} deadA={(sA[mA]==0).sum()} ({(mA).sum()} aktywnych)", flush=True)
