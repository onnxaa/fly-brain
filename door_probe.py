"""Real DoOR profiles: consensus matrix -> glomerulus -> ORN globals. Negatives (inhibition) kept.
NOTE: exploratory probe, superseded by door_real.py (incl. corrected geosmin CAS 16423-19-1)."""
...
# matrix columns -> glomerulus
col2g = {c: unit2glom.get(c) for c in M.columns}
print(f"columns: {len(M.columns)}, mapped: {sum(v is not None for v in col2g.values())}", flush=True)
...
for k, v in CAS.items():
    ...
    print(k, "->", hit["Name"].values if len(hit) else "MISSING", flush=True)
