"""VIS retinotopy map (supp1 positions, two eyes) + ME/LO groups. Appends to roles."""
...
med = np.nanmedian(px); eye = (px > med).astype(int)  # 0/1 - sides
pix = np.zeros(len(VIS), dtype=np.int32)
# EACH eye sees its hemifield: eye0 <- columns 0..31, eye1 <- columns 32..63.
# Row from pos_y (64), in-hemifield position from pos_x (32).
for e in (0, 1):
...
print(f"VIS {len(VIS)} eyes: L={(eye==0).sum()} R={(eye==1).sum()} ME {len(roles['ME'])} LO {len(roles['LO'])}", flush=True)
